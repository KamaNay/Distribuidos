...
public class Server extends Thread {
    ...
    private static class Job {
        ...
        public Job(String skill, int id, String available, String searchable) {
            this.skill = skill;
            this.id = id;
    ...
    private static class Selected {
        ...
        public Selected(int id_company, int id_user) {
            this.id_company = id_company;
            this.id_user = id_user;
        }
                ...
                if (jsonMessage != null) {
                    ...
                    switch (operation) {
                        ...
                        case "UPDATE_JOB":
                            ...
                            break;
                        case "SEARCH_JOB":
                            handleSearchJ(requestJson, out);
                            break;
                        case "SET_JOB_AVAILABLE":
                            handleAvailable(requestJson, out);
                            break;
                        case "SET_JOB_SEARCHABLE":
                            handleSearchable(requestJson, out);
                            break;
                        case "SEARCH_CANDIDATE":
                            handleSearchC(requestJson, out);
                            break;
                        case "CHOOSE_CANDIDATE":
                            handleChooseC(requestJson, out);
                            break;
                        case "GET_COMPANY":
    ...
    private void handleGetCompany(JsonObject request, PrintWriter out) throws IOException {
        String token = request.get("token").getAsString();
        int id;
        try {
            id = JWTValidator.getIdClaim(token);
        } catch (JWTVerificationException e) {
            JsonObject responseJson = Request.createResponse("GET_COMPANY", "INVALID_TOKEN", "");
        ...
        List<Selected> selecteds = readSelectedDatabase();

        for (Selected selected : selecteds) {
            for (Company company : companies) {
                if (selected.getId_user() == id) {
                    int id_company = selected.getId_company();

                    if (id_company == company.getId()) {
        ...
        JsonObject responseJson = Request.createResponse("GET_COMPANY", "SUCCESS", "");
        dataS.add("company", new Gson().toJsonTree(selectedList));
        dataS.addProperty("company_size", String.valueOf(selectedList.size()));
        responseJson.add("data", dataS);
        fileWriter.write("enviado: " + responseJson + "success");
        fileWriter.newLine();
        fileWriter.flush();
        out.println(Request.toJsonString(responseJson));
    ...
    private void handleChooseC(JsonObject request, PrintWriter out) throws IOException {
        String token = request.get("token").getAsString();
        JsonObject data = request.getAsJsonObject("data");
        int id_company;
        int id_user = data.get("id_user").getAsInt();

        try {
            id_company = JWTValidator.getIdClaim(token);
        } catch (JWTVerificationException e) {
            JsonObject responseJson = Request.createResponse("CHOOSE_CANDIDATE", "INVALID_TOKEN", "");
        ...
        for (User user : users) {
            if (user.getId() == id_user) {
                ...
                Selected newsSelected = new Selected(id_company, id_user);
                selecteds.add(newsSelected);
                writeSelectedDatabase(selecteds);

                JsonObject responseJson = Request.createResponse("CHOOSE_CANDIDATE", "SUCCESS", "");
        ...
        JsonObject responseJson = Request.createResponse("CHOOSE_CANDIDATE", "CANDIDATE_NOT_FOUND", "");
    ...
    private void handleSearchC(JsonObject request, PrintWriter out) throws IOException {
        JsonObject data = request.getAsJsonObject("data");

        if (data == null) {
            JsonObject responseJson = Request.createResponse("SEARCH_CANDIDATE", "FAILURE", "Invalid request data");
            out.println(Request.toJsonString(responseJson));
            return;
        }

        List<SkillSet> skillList = readSkillSetDatabase();
        List<Experience> experiences = readExperienceDatabase(); // Carregar as experiências
        List<SkillSet> filteredSkills;
        List<User> users = readUserDatabase();

        // Verifica se possui os campos skill e experience
        boolean hasSkill = data.has("skill");
        boolean hasExperience = data.has("experience");

        if (hasSkill || hasExperience) {
            String skillR = hasSkill ? data.get("skill").getAsString() : null;
            int experienceR = hasExperience ? data.get("experience").getAsInt() : -1;

            if (hasSkill && !hasExperience) {
                filteredSkills = skillList.stream()
                        .filter(skillS -> skillR.equals(skillS.getSkill()))
                        .collect(Collectors.toList());
            } else if (!hasSkill && hasExperience) {
                filteredSkills = skillList.stream()
                        .filter(skillS -> experienceR <= skillS.getExperience())
                        .collect(Collectors.toList());
            } else {
                String filter = data.get("filter").getAsString();
                if ("AND".equalsIgnoreCase(filter)) {
                    filteredSkills = skillList.stream()
                            .filter(skillS -> skillR.equals(skillS.getSkill()) && experienceR <= skillS.getExperience())
                            .collect(Collectors.toList());
                } else if ("OR".equalsIgnoreCase(filter)) {
                    filteredSkills = skillList.stream()
                            .filter(skillS -> skillR.equals(skillS.getSkill()) || experienceR <= skillS.getExperience())
                            .collect(Collectors.toList());
                } else {
                    JsonObject responseJson = Request.createResponse("SEARCH_CANDIDATE", "FAILURE", "Invalid filter");
                    out.println(Request.toJsonString(responseJson));
                    return;
                }
            }

            List<JsonObject> skillSet = new ArrayList<>();
            Set<String> uniqueSkills = new HashSet<>();

            for (User user : users) {
                for (SkillSet skillS : filteredSkills) {
                    for (Experience exp : experiences) {
                        if (exp.getExperience() == skillS.getExperience()) {
                            String uniqueKey = skillS.getSkill() + ":" + exp.getExperience() + ":" + exp.getId_user()
                                    + ":" + user.getName();
                            if (!uniqueSkills.contains(uniqueKey)) {
                                uniqueSkills.add(uniqueKey);
                                JsonObject skillJson = new JsonObject();
                                skillJson.addProperty("skill", skillS.getSkill());
                                skillJson.addProperty("experience", String.valueOf(exp.getExperience()));
                                skillJson.addProperty("id", String.valueOf(skillS.getId()));
                                skillJson.addProperty("id_user", String.valueOf(exp.getId_user()));
                                skillJson.addProperty("name", user.getName());
                                skillSet.add(skillJson);
                            }
                        }
                    }
                }
            ...
            JsonObject responseJson = Request.createResponse("SEARCH_CANDIDATE", "SUCCESS", "");
            JsonObject dataS = new JsonObject();
            responseJson.add("data", dataS);
            dataS.add("profile", new Gson().toJsonTree(skillSet));
            dataS.addProperty("profile_size", String.valueOf(skillSet.size()));
            ...
            JsonObject responseJson = Request.createResponse("SEARCH_CANDIDATE", "FAILURE",
                    "No search criteria provided");
            out.println(Request.toJsonString(responseJson));
        }
    }

    private void handleAvailable(JsonObject request, PrintWriter out) throws IOException {
        JsonObject data = request.getAsJsonObject("data");
        int id_job = data.get("id").getAsInt();
        String available = data.get("available").getAsString();
        ...
        for (Job job : jobs) {
            if (job.getId() == id_job) {
                ...
                responseJson.addProperty("operation", "SET_JOB_AVAILABLE");
    ...
    private void handleSearchable(JsonObject request, PrintWriter out) throws IOException {
        JsonObject data = request.getAsJsonObject("data");
        int id_job = data.get("id").getAsInt();
        String searchable = data.get("searchable").getAsString();
        ...
        for (Job job : jobs) {
            if (job.getId() == id_job) {
                ...
                JsonObject responseJson = new JsonObject();
                responseJson.addProperty("operation", "SET_JOB_SEARCHABLE");
                responseJson.addProperty("status", "SUCCESS");
                responseJson.add("data", new JsonObject());
                fileWriter.write("enviado: " + responseJson + "success");
                fileWriter.newLine();
    ...
    private void handleSearchJ(JsonObject request, PrintWriter out) throws IOException {
        JsonObject data = request.getAsJsonObject("data");

        if (data == null) {
            JsonObject responseJson = Request.createResponse("SEARCH_JOB", "FAILURE", "Invalid request data");
            out.println(Request.toJsonString(responseJson));
            return;
        }

        List<JobSet> jobList = readJobSetDatabase(); // Método que retorna a lista de todas as vagas
        List<JobSet> filteredJobs;
        List<Job> jobS = readJobDatabase();

        for (Job job : jobS) {
            if (job.getSearchable().equals("YES")) {
                if (!data.has("experience")) {
                    String skill = data.get("skill").getAsString();

                    filteredJobs = jobList.stream()
                            .filter(jobs -> skill.equals(jobs.getSkill()))
                            .collect(Collectors.toList());

                    List<JsonObject> jobSet = filteredJobs.stream().map(jobs -> {
                        JsonObject jobJson = new JsonObject();
                        jobJson.addProperty("skill", jobs.getSkill());
                        jobJson.addProperty("experience", String.valueOf(jobs.getExperience()));
                        jobJson.addProperty("id", String.valueOf(jobs.getId()));
                        return jobJson;
                    }).collect(Collectors.toList());

                    JsonObject dataS = new JsonObject();
                    JsonObject responseJson = Request.createResponse("SEARCH_JOB", "SUCCESS", "");
                    responseJson.add("data", dataS);
                    dataS.add("jobset", new Gson().toJsonTree(jobSet));
                    dataS.addProperty("jobset_size", String.valueOf(jobSet.size()));
                ...
                if (!data.has("skill")) {
                    ...
                    filteredJobs = jobList.stream()
                            .filter(jobs -> experience <= jobs.getExperience())
                            .collect(Collectors.toList());
                ...
                if (data.has("skill") && data.has("experience")) {
                    String skill = data.get("skill").getAsString();
                    int experience = data.get("experience").getAsInt();
                    String filter = data.get("filter").getAsString();
                    ...
                    if ("AND".equalsIgnoreCase(filter)) {
                        filteredJobs = jobList.stream()
                                .filter(jobs -> skill.equals(jobs.getSkill()) && experience <= jobs.getExperience())
                                .collect(Collectors.toList());
                    } else if ("OR".equalsIgnoreCase(filter)) {
                        filteredJobs = jobList.stream()
                                .filter(jobs -> skill.equals(jobs.getSkill()) || experience <= jobs.getExperience())
                        ...
                        JsonObject responseJson = Request.createResponse("SEARCH_JOB", "FAILURE", "Invalid filter");
                        out.println(Request.toJsonString(responseJson));
                        return;
                    }
                ...
                JsonObject responseJson = Request.createResponse("SEARCH_JOB", "SUCCESS", "");
                responseJson.add("data", data2);
                @SuppressWarnings("rawtypes")
                ArrayList jobSet = new ArrayList<>();
                data2.add("jobset", new Gson().toJsonTree(jobSet));
                data2.addProperty("jobset_size", "0");
    ...
    private void handleLookupJS(JsonObject requestJson, PrintWriter out) throws SQLException, IOException {
        ...
        try {
            ...
            JsonObject responseJson = Request.createResponse("LOOKUP_JOBSET", "INVALID_TOKEN", "");
            fileWriter.write("enviado: " + responseJson + "invalidtoken");
            fileWriter.newLine();
            fileWriter.flush();
            out.println(Request.toJsonString(responseJson));
        ...
        for (Job job : jobs) {
            for (ExperienceJ experience : experiences) {
                ...
                if (experience.getId_company() == id) {

                    jobData.addProperty("skill", job.getSkill());
                    jobData.addProperty("id", String.valueOf(experience.getId_skill()));
                    jobData.addProperty("available", job.getAvailable());
                    jobData.addProperty("searchable", job.getSearchable());
                    jobData.addProperty("experience", String.valueOf(experience.getExperience()));
        ...
        responseJson.add("data", dataS);
        dataS.add("jobset", new Gson().toJsonTree(jobsList));
        dataS.addProperty("jobset_size", String.valueOf(jobsList.size()));
        ...
        for (Job job : jobs) {
            for (ExperienceJ experience : experiences) {
                if (job.getId() == id_job && experience.getId_company() == id
                    ...
                    data.addProperty("id", String.valueOf(job.getId()));
                    data.addProperty("experience", String.valueOf(experience.getExperience()));
                    data.addProperty("available", job.getAvailable());
                    data.addProperty("searchable", job.getSearchable());
    ...
    private void handleIncludeJ(JsonObject requestJson, PrintWriter out) throws IOException, SQLException {
        ...
        for (Job job : jobDataset) {
            if (job.getSkill().equals(skillName)) {
                ...
                Job newJob = new Job(skillName, id_skill, available, searchable);
                jobs.add(newJob);
                writeJobDatabase(jobs);
    ...
    private List<Selected> readSelectedDatabase() {
        List<Selected> selecteds = new ArrayList<>();
        String query = "SELECT id_company, id_user FROM selected";
        ...
        try (Connection connection = DriverManager.getConnection(URL);
            ...
            while (resultSet.next()) {
                ...
                Selected selected = new Selected(id_company, id_user);
    ...
    private void writeSelectedDatabase(List<Selected> selecteds) {
        String query = "INSERT INTO selected (id_company, id_user) VALUES (?, ?) ON DUPLICATE KEY UPDATE id_company=VALUES(id_company), id_user=VALUES(id_user)";

        try (Connection connection = DriverManager.getConnection(URL);
                PreparedStatement preparedStatement = connection.prepareStatement(query)) {

            for (Selected selected : selecteds) {
                preparedStatement.setInt(1, selected.getId_company());
                preparedStatement.setInt(2, selected.getId_user());
    ...
    private void writeJobDatabase(List<Job> jobs) {
        String query = "INSERT INTO jobs (skill, id, available, searchable) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE skill=VALUES(skill), available=VALUES(available), searchable=VALUES(searchable)";
        ...
        try (Connection connection = DriverManager.getConnection(URL);
            ...
            for (Job job : jobs) {
                ...
                preparedStatement.setString(3, job.getAvailable());
                preparedStatement.setString(4, job.getSearchable());
                preparedStatement.executeUpdate();
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private List<Job> readJobDataset() {
        ...
        try (Connection connection = DriverManager.getConnection(URL);
            ...
            while (resultSet.next()) {
                ...
                int id = resultSet.getInt("id");
                Job job = new Job(skill_name, id, "YES", "YES");
                jobs.add(job);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private List<Job> readJobDatabase() {
        List<Job> jobs = new ArrayList<>();
        String query = "SELECT skill, id, available, searchable FROM jobs";
        ...
        try (Connection connection = DriverManager.getConnection(URL);
            ...
            while (resultSet.next()) {
                ...
                String searchable = resultSet.getString("searchable");
                Job job = new Job(skill_name, id, available, searchable);
                jobs.add(job);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private void handleLookupSS(JsonObject requestJson, PrintWriter out) throws SQLException, IOException {
        ...
        while (rs.next()) {
            ...
            skillData.addProperty("experience", String.valueOf(rs.getInt("experience"))); // Ajuste o nome da coluna
                                                                                          // conforme necessário
        ...
        responseJson.add("data", dataS);
        dataS.add("skillset", new Gson().toJsonTree(skillsList));
        dataS.addProperty("skillset_size", String.valueOf(skillsList.size()));
    ...
    public static void createTables() {
        ...
        String createExperienceJTableSQL = "CREATE TABLE IF NOT EXISTS experienceJ (" +
                ...
                "experience INT NOT NULL, " +
                "id_skill INT NOT NULL)";
        String createSelectedUsersTableSQL = "CREATE TABLE IF NOT EXISTS selected (" +
                "id INT AUTO_INCREMENT PRIMARY KEY, " +
                "id_company INT NOT NULL, " +
                "id_user INT NOT NULL)";
        ...
        try (Connection connection = getConnection();
            ...
            statement.execute(createSelectedUsersTableSQL);
            System.out.println("Tabela'selected' criada com sucesso.");
    ...
    public static void main(String[] args) {
        ...
        try (BufferedWriter fileWriter = new BufferedWriter(new FileWriter("server_log.txt", true));
            ...
            while (true) {
                ...
                try (BufferedWriter write = new BufferedWriter(new FileWriter("logins.txt", true))) {
                    write.write(clientSocket.getInetAddress().toString());
                    write.newLine();
                    write.flush();
                }
