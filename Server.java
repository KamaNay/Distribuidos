...
public class Server extends Thread {
    ...
    private void handleGetCompany(JsonObject request, PrintWriter out) throws IOException {
        ...
        List<JsonObject> selectedList = new ArrayList<>();
            ...
            for (Company company : companies) {
                if (selected.getId_user() == id) {
                    ...
                    if (id_company == company.getId()) {
                        ...
                        dataS.addProperty("company_size", selectedList.size());
                        fileWriter.write("enviado: " + responseJson + "success");
                        fileWriter.newLine();
                        fileWriter.flush();
                        out.println(Request.toJsonString(responseJson));
        ...
        JsonObject responseJson = Request.createResponse("GET_COMPANY", "SUCCESS", "");
        responseJson.add("company", new Gson().toJsonTree(selectedList));
        responseJson.addProperty("company_size", selectedList.size());
        fileWriter.write("enviado: " + responseJson + "success");
        fileWriter.newLine();
        fileWriter.flush();
        out.println(Request.toJsonString(responseJson));
    ...
    private void handleSearchC(JsonObject request, PrintWriter out) throws IOException {
        ...
        if (hasSkill || hasExperience) {
            ...
            Set<String> uniqueSkills = new HashSet<>(); // Conjunto para armazenar combinações únicas de skill,
                                                        // experience e id_user
                ...
                for (Experience exp : experiences) {
                    if (exp.getExperience() == skillS.getExperience()) {
                        ...
                        if (!uniqueSkills.contains(uniqueKey)) {
                            ...
                            skillJson.addProperty("experience", exp.getExperience());
                            skillJson.addProperty("id", skillS.getId());
                            skillJson.addProperty("id_user", exp.getId_user());
            ...
            dataS.addProperty("profile_size", skillSet.size());
            ...
            if (job.getSearchable().equals("YES")) {
                if (!data.has("experience")) {
                    ...
                    List<JsonObject> jobSet = filteredJobs.stream().map(jobs -> {
                        ...
                        jobJson.addProperty("experience", jobs.getExperience());
                        jobJson.addProperty("id", jobs.getId());
                        return jobJson;
                    }).collect(Collectors.toList());
                    ...
                    dataS.addProperty("jobset_size", jobSet.size());
                ...
                JsonObject responseJson = Request.createResponse("SEARCH_JOB", "NO JOBS", "No jobs searchable");
                out.println(Request.toJsonString(responseJson));
                return;
            }
        }
    ...
    private void handleLookupJS(JsonObject requestJson, PrintWriter out) throws SQLException, IOException {
        ...
        responseJson.add("data", dataS);
        dataS.add("skills", new Gson().toJsonTree(jobsList));
        dataS.addProperty("jobset_size", jobsList.size());
    ...
    private void handleLookupSS(JsonObject requestJson, PrintWriter out) throws SQLException, IOException {
        ...
        responseJson.add("data", dataS);
        dataS.add("skills", new Gson().toJsonTree(skillsList));
        dataS.addProperty("skillset_size", skillsList.size());
